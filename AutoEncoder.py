from tensorflow.keras.layers import Input, Dense, Conv1D, Conv2D, Dropout, ZeroPadding2D, SeparableConv2D, UpSampling2D, BatchNormalization, ReLU, LSTM
...
def FullChannelEncoder(encoded_feature_num,inputs, window_size = 2, dilated = [1,2,4,8,16,32], freq = 256, 
	...
	for df in dilated:
		...
		x = Conv2D(filters=8, kernel_size=(1,3),activation='relu',padding='same')(x)	# (None, 23, 256, 16)
		x = MaxPooling2D((1,2))(x)	# (None, 23, 128, 16)
		x = Conv2D(filters=16, kernel_size=(1,3),activation='relu',padding='same')(x)	# (None, 23, 128, 32)
	...
	x=  tf.squeeze(x, axis = -3) # (None, 64, 21)
	#x = Flatten()(x)
	#x = Dense(encoded_feature_num)(x)
	x = LSTM(8, return_sequences=True)(x)	# (None, 64 , 8)
...
def FullChannelDecoder(inputs, dilated = [1,2,4,8,16,32], pooling_rate = 8, freq = 256, window_size = 2):
	# inputs = (None, 64, 8)
	x = LSTM(21, return_sequences=True)(inputs)	# inputs = (None, 64, 21)
	#x = Dense(64*21,activation='relu')(x)
	...
	for i in range(len(dilated)):
		...
		x = UpSampling2D(size=(1,2))(x)
		x = Conv2DTranspose(filters=8,kernel_size=(1,3),activation='relu',padding='same')(x)
		x = UpSampling2D(size=(1,2))(x)
