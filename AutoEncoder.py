...
def FullChannelEncoder(encoded_feature_num,inputs, window_size = 2, dilated = [1,2,4,8,16,32], freq = 256, 
	...
	cnt = 0
	for df in dilated:
		x = Conv2D(filters=8, kernel_size = (1,8),padding='same',activation='relu',dilation_rate=(1,df))(inputs)	# (None, 23, 512, 8)
		x = MaxPooling2D((1,2))(x)	# (None, 23, 256, 8)
		x = Conv2D(filters=8, kernel_size=(1,8),activation='relu',padding='same')(x)	# (None, 23, 256, 16)
		x = MaxPooling2D((1,2))(x)	# (None, 23, 128, 16)
		x = Conv2D(filters=16, kernel_size=(1,8),activation='relu',padding='same')(x)	# (None, 23, 128, 32)
...
def FullChannelDecoder(inputs, dilated = [1,2,4,8,16,32], pooling_rate = 8, freq = 256, window_size = 2):
	...
	for i in range(len(dilated)):
		...
		x = Conv2DTranspose(filters=16,kernel_size=(1,8),activation='relu',padding='same')(x)
		x = UpSampling2D(size=(1,2))(x)
		x = Conv2DTranspose(filters=8,kernel_size=(1,8),activation='relu',padding='same')(x)
		x = UpSampling2D(size=(1,2))(x)
		x = Conv2DTranspose(filters=8,kernel_size=(1,8),padding='same',activation='relu',dilation_rate=dilated[i])(x)
		x_list.append(x)
		sum += 6-cnt
		cnt+=1
