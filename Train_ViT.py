...
def train(model_name, encoder_model_name, channel, data_type = 'snu'):
    ...
    for patient_idx, patient_name in enumerate(filtered_interval_dict.keys()) :
        ...
        for idx, set in enumerate(train_val_sets):
            ...
            checkpoint_path = f"./mViT/{model_name}/{patient_name}/set{idx+1}/cp.ckpt"
            checkpoint_dir = os.path.dirname(checkpoint_path)
        ...
        # 상대적으로 데이터 갯수가 적은 것들은 window_size 2초에 sliding_size 1초로 overlap 시켜 데이터 증강
            ...
            full_model = one_channel_mobile_vit(image_size=(scale_resolution, int(sr * window_size),1))
            ...
            del full_model
            #del dilation_output
            matrix, postprocessed_matrix, sens, fpr, seg_results = TestFullModel_specific.validation(checkpoint_path, val_intervals, data_type, 1,1, window_size=window_size, channel=channel, preprocessing=PreProcessing.SegmentsCWT)
