...
def GetBatchIndexes(data_len, batch_num):
    ...
    idx_list = [ list(range(int(i*batch_seg_size), int((i+1)*batch_seg_size))) for i in range(batch_num*mul) ]
    random.shuffle(idx_list)

    batch_idx_mask = []
    for i in range(batch_num):
        batch_idx_mask.append(np.asarray(np.concatenate(idx_list[int(mul*i) : int(mul*(i+1))]),dtype=int ))
